...
Storm.prototype = {
  ...
  /**
   * Send a sync message. Usually a response to a heartbeat.
   */
  sync: function() {
    this.sendCommand({
      command: 'sync'
  ...
  emit: function(anchor, tuple, options) {
    ...
    // Bad arity
      ...
      var msg = "Incorrect arguments to `emit`: " + JSON.stringify([].slice.call(arguments));
      this.log(msg);
      throw new Error(msg);
    }
  ...
  parseMessage: function(msg) {
    ...
    if (this.msgCount == 1) {
      ...
      if (this.mode == 'spout') {
        ...
        // Heartbeats are sent from 0.9.3 forward.
        if (cmd.stream === '__heartbeat') {
          this.sync();
        } else if ('tuple' in cmd) {
